...
import { IClient, EventModel, IntercomSearch } from '../typings';
import { determineIfSearchByUserId, determineIfSearchByIntercomUserId, determineIfSearchByEmail } from '../utils';
...
export class IntercomEventsAPI {
	...
	public async list(query: IntercomSearch): Promise<AxiosResponse<any>> {
		if (determineIfSearchByUserId(query)) return await this._client.get('/events', { type: 'user', user_id: query.user_id });
